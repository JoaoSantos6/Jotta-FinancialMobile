...
- **Trava de CI de ofuscação/R8** (RNF-14, item 3). Não existe build de release nem
...
| RNF-14 | CI reprova manifest com `INTERNET` ou `allowBackup ≠ false` | **Parcial** — itens 1, 2 e 4 entram; item 3 (ofuscação) é do M6 |
| RNF-16 | Chave nunca em log, backup ou preferências em claro | **Total** — inclui o `grep` de CI |
| RNF-9 | Zero perda em atualização de versão | **Parcial** — a estratégia de migração é estabelecida com a v1; o teste de migração com base populada só existe quando houver uma v2 |
...
> Este critério substitui o texto original do marco M0 no guarda-chuva ("banco extraído
> do emulador não abre sem a chave"). A prova passa a rodar no *desktop*, na CI, a cada
> push, em vez de exigir um emulador Android no pipeline. A verificação no aparelho de
> verdade continua existindo, como item do checklist manual do marco. Decisão registrada
> em `DECISIONS.md`.
...
e a segunda abertura recupera exatamente a mesma chave. Nenhum log, em nenhum nível,
contém o material da chave.
...
ou que logue um valor monetário fora de bloco de debug, quando a CI roda, então o build
**falha**. Cada uma das três travas é demonstrada uma vez com uma violação proposital
...
| A2 | `libsqlcipher` na CI Linux divergir da biblioteca embarcada no APK Android, fazendo o teste passar no desktop e o app falhar no aparelho | Alto | Versão da lib fixada nos dois lados; verificação no aparelho no checklist manual de fim de marco |
...
**Nenhuma.** As três que bloqueavam esta fatia foram fechadas em 2026-09-05:
`applicationId`, estratégia de prova do SEG-1 na CI e escopo de tela do M0. Estão em
`DECISIONS.md`.

Seguem em aberto no projeto, mas **não bloqueiam este ADR** — todas caem no M5 ou
depois: `FLAG_SECURE`, export CSV em texto plano, recorrência automática vs. sugerida,
identidade visual.
