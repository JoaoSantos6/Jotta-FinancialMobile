...
| 2026-09-06 | `dynamic_color` travado em **1.9.0**, não a versão mais recente (2.1.0) | Usar a versão mais recente resolvida automaticamente | Claude (a partir da 2.0.0, `dynamic_color` passou a expor `ColorScheme` de um novo pacote `material_ui` — extração do Material para fora do SDK do Flutter, ainda em transição — em vez do `ColorScheme` de `package:flutter/material.dart`. Os dois tipos não são intercambiáveis: `ThemeData(colorScheme:)` da SDK pinada recusa o tipo do `material_ui`. 1.9.0 é a última versão que usa o `ColorScheme` padrão do SDK) |
...
| 2026-09-06 | `compileSdk` literal **37** em `android/app/build.gradle.kts`, não `flutter.compileSdkVersion` | Deixar o padrão do Flutter resolver | Claude (achado em CI real, no primeiro push com o Actions rodando: `flutter build apk --debug` falhou porque `flutter_secure_storage` 11.0.0 exige `compileSdk 37`, e o Flutter 3.47.2 resolve `flutter.compileSdkVersion` para 36. Mesma lógica do `minSdk`, já literal desde a T-01: o padrão do Flutter muda de versão para versão sem aviso no nosso changelog. Travado por teste — V-01 estendido) |
| 2026-09-06 | `com.jotta.financial.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION` aceito em `tool/ci/permissions.baseline` | Investigar como se fosse uma permissão de terceiro | Claude (achado em CI real: não é permissão de capacidade nem de dependência — é self-permission de assinatura que a própria androidx.core injeta em todo app com compileSdk >= 33, exigência do Android 13 para BroadcastReceiver não-exportado registrado em runtime. Não concede acesso a nada fora do app) |
| 2026-09-06 | `android.permission.INTERNET` apareceu no manifest merged real, de origem ainda não identificada | Aceitar no baseline sem investigar, já que "provavelmente é inofensiva" | Claude (SEG-5 é a garantia mais forte do app — zero rede é reforçada pelo SO, não por promessa de código. Mantido **fora** do baseline; a CI segue reprovando o build até a origem ser identificada. Diagnóstico em andamento via `manifest-merger-debug-report.txt`, que mostra a proveniência de cada permissão) |
| 2026-09-06 | T-46 a T-49 do ADR-1 (build do APK, travas de manifest) marcadas `[~]` em `TASK.md`, não `[x]` | Marcar como concluídas com base na simulação | Claude (o ambiente de implementação não tem Android SDK: `dl.google.com` está bloqueado pela política de rede do sandbox, e os pacotes `android-sdk-*` do apt do Ubuntu vão só até a API 23 — incompatíveis com o `compileSdk` 34+/`aapt2` que o Flutter/AGP atuais exigem. `check_manifest.sh` foi validado com um `aapt2` falso reproduzindo os 5 cenários esperados — limpo, `INTERNET`, `VIBRATE` fora do baseline, `allowBackup='true'`, `dataExtractionRules` ausente — mas a confirmação real, contra um APK de verdade, só acontece quando este branch for pushado e o Actions do GitHub rodar com Android SDK e rede completos no runner hospedado) |
